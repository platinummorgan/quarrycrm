...
// eslint-disable-next-line no-console
console.log('__tests__/setup.ts: TEST_DATABASE_URL=', process.env.TEST_DATABASE_URL)
...
if (!process.env.DATABASE_URL || !/test/i.test(process.env.DATABASE_URL)) {
  ...
  // eslint-disable-next-line no-console
  console.warn('⚠️  ALLOW_UNSAFE_TEST_DB=1 is set — tests will run against a non-test DB. Proceed with caution.')
}
