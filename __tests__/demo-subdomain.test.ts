...
describe('Demo Subdomain Middleware', () => {
  ...
  describe('Write operation blocking', () => {
    it('should block POST on demo subdomain', () => {
      ...
      const shouldBlock = isDemoSubdomain && writeMethod && pathname.startsWith('/api/') && !isAllowed;
  ...
  describe('Belt-and-suspenders approach', () => {
    ...
    it('should block if either check fails', () => {
      const scenarios = [
        { subdomain: true, user: false, shouldBlock: true, reason: 'Subdomain block' },
        { subdomain: false, user: true, shouldBlock: true, reason: 'User block' },
        { subdomain: true, user: true, shouldBlock: true, reason: 'Both block' },
        { subdomain: false, user: false, shouldBlock: false, reason: 'Neither block' },
      ];
