...
describe('PII Masking', () => {
  describe('maskEmail', () => {
    ...
    it('should handle custom options', () => {
      expect(maskEmail('john.doe@example.com', { showStart: 3 })).toBe(
        'joh*****@example.com'
      )
      expect(maskEmail('john.doe@example.com', { maskChar: 'x' })).toBe(
        'joxxxxxx@example.com'
      )
      expect(
        maskEmail('john.doe@example.com', { preserveStructure: false })
      ).toBe('jo******@example.com')
    })
  ...
  describe('maskPhone', () => {
    ...
    it('should handle custom options', () => {
      expect(maskPhone('+1 (555) 123-4567', { showEnd: 2 })).toBe(
        '+* (***) ***-**67'
      )
      expect(maskPhone('+1 (555) 123-4567', { maskChar: 'x' })).toBe(
        '+x (xxx) xxx-4567'
      )
      expect(maskPhone('+1 (555) 123-4567', { preserveStructure: false })).toBe(
        '*******4567'
      ) // 7 digits masked + 4 shown
    })
