...
interface ConflictResolutionModalProps {
  ...
  onResolve: (id: string, resolution: 'local' | 'remote' | 'merged', auditEntry?: any) => void
  onClose: () => void
}
  ...
  const renderFieldComparison = (field: string, localValue: any, remoteValue: any) => {
    ...
    return (
      <div key={field} className="grid grid-cols-3 gap-4 py-2 border-b border-gray-100 last:border-b-0">
        ...
        <div className={`text-sm p-2 rounded ${isDifferent ? 'bg-red-50 text-red-700' : 'bg-gray-50'}`}>
          {JSON.stringify(localValue)}
        </div>
        <div className={`text-sm p-2 rounded ${isDifferent ? 'bg-green-50 text-green-700' : 'bg-gray-50'}`}>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  <span className="font-medium">Entity ID:</span> {conflict.entityId}
                </div>
                <div>
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Compare local changes with remote data. Fields with differences are highlighted.
              </p>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-3 gap-4 mb-4 text-sm font-medium">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Badge variant="outline" className="text-xs">Your Changes</Badge>
                </div>
                <div className="flex items-center space-x-2">
                  <span>Remote Data</span>
                  <Badge variant="outline" className="text-xs">Server Data</Badge>
                </div>
              </div>
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Choose how to resolve this conflict. Last-write-wins policy will be applied.
              </p>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                ...
                <div className="flex items-center space-x-3">
                  <input
                    ...
                    onChange={(e) => setSelectedResolution(e.target.value as 'merged')}
                  ...
                  <label htmlFor="merged" className="flex-1 opacity-50">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="font-medium">Merge Data (Coming Soon)</span>
                    </div>
                    <p className="text-sm text-muted-foreground mt-1">
                      Intelligently merge conflicting fields (not yet available).
                    </p>
                  </label>
                </div>
              </div>
