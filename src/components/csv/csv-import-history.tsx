...
import { History, Undo2, CheckCircle, XCircle, AlertTriangle, Clock } from 'lucide-react'
import { toast } from 'sonner'

type ImportStatus = 'PENDING' | 'PROCESSING' | 'COMPLETED' | 'FAILED' | 'ROLLED_BACK'
...
export function CsvImportHistory({ onRollback }: CsvImportHistoryProps) {
  ...
  const getStatusInfo = (status: ImportStatus) => {
    switch (status) {
      ...
      case 'ROLLED_BACK':
        return { icon: Undo2, color: 'text-orange-600', bgColor: 'bg-orange-50' }
      case 'PROCESSING':
      ...
      <CardContent>
        {imports.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p>No imports yet</p>
            <p className="text-sm">Import history will appear here after your first CSV import</p>
          </div>
        ) : (
                ...
                <div key={importItem.id} className="border rounded-lg p-4">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <div className="flex items-center space-x-2">
                          ...
                          <Badge variant="outline">{formatStatus(importItem.status)}</Badge>
                        </div>
                        <p className="text-sm text-muted-foreground">
                          {new Date(importItem.createdAt).toLocaleString()}
                          {importItem.completedAt && importItem.completedAt !== importItem.createdAt && (
                            <> • Completed {new Date(importItem.completedAt).toLocaleString()}</>
                          )}
                        </p>
                      </div>
                    </div>

                    {importItem.rollbackAvailable && importItem.status === 'COMPLETED' && (
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              ...
                              {importItem.rollbackData?.rollbackAction === 'DELETE'
                                ? 'delete'
                                : 'restore'
                              }{' '}
                              {importItem.rollbackData?.affectedIds.length || 0} records.
                  ...
                  <div className="grid grid-cols-4 gap-4 mb-3">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-600">{importItem.processedRows}</div>
                      <div className="text-xs text-muted-foreground">Processed</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-blue-600">{importItem.totalRows}</div>
                      <div className="text-xs text-muted-foreground">Total</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-yellow-600">{importItem.skippedRows}</div>
                      <div className="text-xs text-muted-foreground">Skipped</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-red-600">{importItem.errorRows}</div>
                        ...
                        <ul className="text-sm space-y-1">
                          ...
                          {importItem.errors.length > 3 && (
                            <li className="text-muted-foreground">
                              • ...and {importItem.errors.length - 3} more errors
                            </li>
                          )}
                        </ul>
                      </AlertDescription>
