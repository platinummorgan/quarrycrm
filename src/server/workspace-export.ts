...
export async function processExportJob(jobId: string) {
  ...
  try {
    ...
    if (metadata.format === 'json') {
      zip.file(
        'contacts.jsonl',
        exportData.contacts.map((c) => JSON.stringify(c)).join('\n')
      )
      zip.file(
        'companies.jsonl',
        exportData.companies.map((c) => JSON.stringify(c)).join('\n')
      )
      zip.file(
        'deals.jsonl',
        exportData.deals.map((d) => JSON.stringify(d)).join('\n')
      )
      zip.file(
        'pipelines.jsonl',
        exportData.pipelines.map((p) => JSON.stringify(p)).join('\n')
      )
      zip.file(
        'activities.jsonl',
