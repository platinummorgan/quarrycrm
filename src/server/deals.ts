...
import { dealsFiltersSchema, moveDealSchema, type DealsListResponse, type PipelinesListResponse } from '@/lib/zod/deals'
import { revalidatePath } from 'next/cache'
import { PerformanceUtils } from '@/lib/metrics'

// Server action to get deals list
export async function getDeals(
  filters: { pipeline?: string; q?: string; limit?: number; cursor?: string } = {}
): Promise<DealsListResponse> {
  return PerformanceUtils.measureServerOperation('deals-list', async () => {
    const { orgId } = await requireOrg()
    const { pipeline, q, limit = 25, cursor } = dealsFiltersSchema.parse(filters)
  ...
  }, { query: filters.q, pipeline: filters.pipeline, limit: filters.limit }).then(({ result }) => result)
}
...
export async function moveDealToStage(data: { dealId: string; stageId: string }) {
  const { orgId } = await requireOrg()
