 ...
 */
export function maskEmail(email: string | null | undefined, options: MaskOptions = {}): string {
 ...
 * // Returns: '***-***-**67'
 */
export function maskPhone(phone: string | null | undefined, options: MaskOptions = {}): string {
...
export function maskPII<T extends Record<string, any>>(
  ...
  for (const field of fields) {
    ...
    if (fieldStr.includes('email')) {
    ...
    } else if (fieldStr.includes('phone') || fieldStr.includes('mobile') || fieldStr.includes('tel')) {
      ...
      if (str.length <= showStart + showEnd) {
        masked[field] = (str[0] + maskChar.repeat(Math.max(1, str.length - 1))) as any;
      } else {
        const visible = str.substring(0, showStart) + maskChar.repeat(str.length - showStart - showEnd) + str.substring(str.length - showEnd);
...
export function autoMaskPII<T extends Record<string, any>>(
  ...
  for (const key in data) {
    if (Object.prototype.hasOwnProperty.call(data, key)) {
      ...
      if (COMMON_PII_FIELDS.some((piiField) => keyLower.includes(piiField.toLowerCase()))) {
