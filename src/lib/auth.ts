...
export const authOptions: NextAuthOptions = {
  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        console.log(
          '🔍 Demo provider authorize called with credentials:',
          !!credentials?.token
        )
    ...
    EmailProvider({
      ...
      async sendVerificationRequest({ identifier, url }) {
        ...
        const emailFrom =
          process.env.EMAIL_FROM?.trim() || 'noreply@mail.quarrycrm.com'
        ...
        try {
          ...
          if (result.error) {
            ...
            throw new Error(
              `Email send failed: ${result.error.message || 'Unknown error'}`
            )
          }

          console.log(
            '✅ Magic link sent successfully, email ID:',
            result.data?.id
          )
  ...
  callbacks: {
    async signIn({ user, account, profile, email, credentials }) {
      ...
      if (account?.provider === 'email' && user?.id && user?.email) {
        console.log(
          '🔍 Email provider sign-in, checking organization membership'
        )
        ...
        try {
          ...
          if (!existingMembership) {
            console.log(
              '🔍 User has no organization, creating default organization'
            )
    ...
    async redirect({ url, baseUrl }) {
      ...
      if (
        urlObj.pathname === '/auth/signin' &&
        !urlObj.searchParams.has('error')
      ) {
        console.log(
          '🔍 Redirecting from sign-in to /app/contacts after successful auth'
        )
    ...
    async session({ session, token }) {
      ...
      try {
        ...
        if (userId) {
          ...
          ;(session.user as any).currentOrganizationId =
    ...
    async jwt({ token, user, account, trigger }) {
      ...
      if (user) {
        ...
        if (account?.provider === 'email') {
          console.log(
            '🔍 Email sign-in detected, fetching organizations for user:',
            user.id
          )
